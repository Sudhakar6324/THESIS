"""run this command  (python3 siren_net.py   --epochs 400   --datapath path_to_data  --dataset_name  with which name model has to save    --vti_name output filename) to generate predicted_vti file."""
import numpy as np
import torch
...
def getImageData(actual_img, val, n_pts, var_name, isMaskPresent, mask_arr):
    ...
    return img
def get_args():
    parser = argparse.ArgumentParser(description="3D Histogram Model Training")

    parser.add_argument('--n_neurons', type=int, default=320)
    parser.add_argument('--n_layers', type=int, default=6)
    parser.add_argument('--epochs', type=int, required=True)
    parser.add_argument('--batchsize', type=int, default=512)
    parser.add_argument('--lr', type=float, default=0.00005)

    parser.add_argument('--no_decay', action='store_true')  # boolean flag
    parser.add_argument('--decay_rate', type=float, default=0.8)
    parser.add_argument('--decay_at_interval', action='store_true')
    parser.add_argument('--decay_interval', type=int, default=15)

    parser.add_argument('--datapath', type=str, required=True)# input file name 
    parser.add_argument('--outpath', type=str, default='./models/')
    parser.add_argument('--exp_path', type=str, default='../logs/')
    parser.add_argument('--modified_data_path', type=str, default='./data/')
    parser.add_argument('--dataset_name', type=str, required=True) # to store model name 
    parser.add_argument('--vti_name', type=str, required=True)# outputfile name
    parser.add_argument('--vti_path', type=str, default='./data/')
...
for n_neurons in range(150, 321, 10):
    ...
    # Initialize model, optimizer, and loss tracker
    model = MyResidualSirenNet(obj).to(device)
    print(model)
    ...
    model_name = f'best_model_{n_neurons}neurons.pth'
    torch.save({"epoch": best_epoch, "model_state_dict": best_model_state}, os.path.join(outpath, model_name))
    print(f'\nSaved best model for {n_neurons} neurons at epoch {best_epoch} | Loss: {best_loss:.6f}\n', flush=True)
    ...
    for i in range(len(prediction_list[0])):
        for j in range(total_vars):
            extracted_list[j].append(prediction_list[j][i].detach().numpy())
    for j in range(total_vars):
    ...
    print(f"[{n_neurons} neurons] RMSE: {rmse:.6f}\n", flush=True)
...
# print(os.path.getsize('/content/models/train_3d_data_200ep_6rb_320n_512bs_5e-05lr_Truedecay_0.8dr_decayingAtInterval15.pth') / (1024 ** 2), 'MB',flush=True)
