...
for epoch in tqdm(range(epochs), desc="Training"):
    ...
    if (epoch + 1) in [100, 200, 300, 400, 500]:
        ...
        save_vti(preds, f"leaky_relu_epoch_{epoch+1}.vti")
...
# Save best model
torch.save(best_model_state, "best_model_leaky_relu.pth")
...
with torch.no_grad():
    preds = model(X_tensor).cpu().numpy()
save_vti(preds, "leaky_relu_epoch_best.vti")
print("Best model VTI file saved as 'leaky_relu_epoch_best.vti'", flush=True)
