...
args = Namespace(
    ...
    decay_interval=15,
    datapath='sklearn_isabel_gmm_gpu3.vti',  # Required: Set the path to your data
    outpath='./models/',
    exp_path='../logs/',
...
for n_neurons in range(150,331,20):
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for X_train, y_train in train_dataloader:
            ...
            classification_loss = F.kl_div(
                F.log_softmax(pred_cls_logits, dim=1), target_cls_probs, reduction='batchmean'
            )

            # Compute gradients separately
            regression_grad = torch.autograd.grad(regression_loss, model.parameters(), retain_graph=True, create_graph=True)
            classification_grad = torch.autograd.grad(classification_loss, model.parameters(), retain_graph=True, create_graph=True)

            # Compute total gradient norm for each
            def grad_norm(grads):
                return torch.sqrt(sum((g.norm() ** 2 for g in grads if g is not None)))

            reg_norm = grad_norm(regression_grad)
            cls_norm = grad_norm(classification_grad)

            # Dynamic alpha based on inverse gradient norm
            alpha = cls_norm / (reg_norm + cls_norm + 1e-8)  # Avoid div-by-zero
        ...
        print(
            f"Epoch {epoch+1}/{MAX_EPOCH} | Loss: {epoch_loss:.6f} | Alpha (cls): {1 - alpha.item():.3f} | "
            f"Time: {round(epoch_end_time - epoch_start_time, 2)}s | LR: {optimizer.param_groups[0]['lr']:.5f}"
        )
    ...
    print(f"\n[Weight Metrics] Evaluation for {n_neurons} neurons:", flush=True)
    ...
    for i, var in enumerate(variables):
        if 'Weight' in var:
            ...
            # === Correlation ===
            corr = pearsonr(gt, pred)[0]

            # === KL Divergence (histogram-based) ===
            p_hist, _ = np.histogram(pred, bins=50, range=(0,1), density=True)
            q_hist, _ = np.histogram(gt, bins=50, range=(0,1), density=True)
            p_hist += 1e-9  # avoid log(0)
            q_hist += 1e-9
            ...
            print(f"{var} | Correlation: {corr:.4f} | KL Divergence: {kl_div:.6f} | MSE: {mse:.6f}", flush=True)
