...
for epoch in tqdm(range(epochs), desc="Training"):
    ...
    if (epoch + 1) in [100, 200, 300, 400, 500]:
        ...
        save_vti(preds, f"Isabel_3D_Predicted_Epoch{epoch+1}_gelu.vti")
...
# Save best model
torch.save(best_model_state, "best_model_gelu.pth")
...
with torch.no_grad():
    preds = model(X_tensor).cpu().numpy()
save_vti(preds, "Isabel_3D_Predicted_Best_gelu.vti")
print("Best model VTI file saved as 'Isabel_3D_Predicted_Best_gelu.vti'", flush=True)
