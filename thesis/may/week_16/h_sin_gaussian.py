...
# H-SIREN First Layer
class HSirenFirstLayer(nn.Module):
    ...
    def forward(self, x):
        out = self.omega_0 * self.linear(x)
        # H-SIREN activation: sin(sinh(2x))
        return torch.sin(torch.sinh(2 * out))

# SIREN Intermediate Layer
...
# Residual SINE Block (no bottleneck)
...
# H-SIREN Residual Network
class MyResidualHSirenNet(nn.Module):
    def __init__(self, num_layers, neurons_per_layer, num_input_dim, num_output_dim):
        super(MyResidualHSirenNet, self).__init__()
        self.Omega_0 = 30
        ...
        for idx in range(self.n_layers):
            ...
            # First layer with H-SIREN activation
            if idx == 0:
                self.net_layers.append(HSirenFirstLayer(layer_in, layer_out, bias=True, omega_0=self.Omega_0))
