...
Original file is located at
    https://colab.research.google.com/drive/1H4E99U_cJAEMbJ9QKrljOfcnpX0h_TNw
"""
...
args = Namespace(
    ...
    decay_interval=15,
    datapath='data/new_GT_teardrop_Gaussian.vti',  # Required: Set the path to your data
    outpath='./models/',
    exp_path='../logs/',
    modified_data_path='./data/',
    dataset_name='3d_data',  # Required: Set the dataset name
    vti_name='predicted_Isabel_Gaussian.vti',  # Required: Name of the dataset
    vti_path='./data/'
)
...
for epoch in tqdm(range(MAX_EPOCH)):
    ...
    # Learning rate decay
    if decay:
        if  not decay_at_equal_interval:
...
for epoch in tqdm(range(MAX_EPOCH)):
    ...
    print(
        f"Epoch: {epoch + 1}/{MAX_EPOCH} | Train Loss: {train_loss_list[-1]} | "
        f"Time: {round(end - start, 2)}s ({device}) | LR: {optimizer_std.param_groups[0]['lr']}"
    )
