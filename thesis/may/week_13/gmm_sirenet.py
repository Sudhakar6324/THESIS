...
torch_vals_means = torch_vals[:, 0:3]
torch_vals_stds= torch_vals[:, 3:6]
torch_vals_weights=torch_vals[:,6:]
print(torch_vals_means.shape)
print(torch_vals_stds.shape)
print(torch_vals_weights.shape)
...
train_dataloader_w = DataLoader(
    TensorDataset(torch_coords, torch_vals_weights),
    batch_size=BATCH_SIZE, pin_memory=True, shuffle=True,
    num_workers=0, generator=g
)
...
weights_id="gmm_weights"
train_single_target(weights_id, train_dataloader_w,obj)

# Paths to saved best models
model_paths = [
    os.path.join(outpath, f"best_model_{mean_id}.pth"),
    os.path.join(outpath, f"best_model_{std_id}.pth"),
    os.path.join(outpath, f"best_model_{weights_id}.pth")
]
