...
no_of_samples = 500
scale_factor = 2
input_file = "new_teardrop_gaussian.vti"    
output_file = "new_gmm_teardrop_gmm.vti"   # Output filename
...
def fit_gmm_for_point(mu, sigma, scale_factor, n_samples=500):
    if sigma<=1e-3 or np.isnan(sigma) or np.isinf(sigma):
        sigma = 1e-3
    sigma = max(sigma * scale_factor, 1e-3)
    
    samples = np.random.normal(mu, sigma, size=n_samples).reshape(-1, 1)
    gmm = GaussianMixture(n_components=3, covariance_type='full', max_iter=50, random_state=42)
    gmm.fit(samples)
