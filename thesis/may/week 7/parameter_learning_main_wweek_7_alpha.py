# -*- coding: utf-8 -*-
"""parameter_learning_main_wweek_7_alpha.ipynb
...
class MyResidualSirenNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat([means, stds, weights], dim=1)  # [B, 9]
...
# ========== Config ==========
neuron_settings = [150,300,350,400,500,]  # keep fixed
alpha_settings = [1]  # values to fine-tune
all_total_losses = {}
all_psnrs = {}
all_weight_losses = {}
...
# ========== Training Function ==========
def train_and_evaluate(n_neurons, alpha):
    print(f"\n==== Training with {n_neurons} Neurons | alpha = {alpha} ====")

    obj['n_neurons'] = n_neurons
    model = MyResidualSirenNet(obj).to(device)
    optimizer = optim.Adam(model.parameters(), lr=LR, betas=(0.9, 0.999))
    criterion = nn.MSELoss()
    log_sigma_means = torch.nn.Parameter(torch.tensor(0.0, requires_grad=True))
    log_sigma_stds = torch.nn.Parameter(torch.tensor(0.0, requires_grad=True))
    log_sigma_weights = torch.nn.Parameter(torch.tensor(0.0, requires_grad=True))
    params = list(model.parameters()) + [log_sigma_means, log_sigma_stds, log_sigma_weights]
    ...
    f"mean_loss:{np.mean(temp_means):.6f}  "
    f"std_loss:{np.mean(temp_stds):.6f}  "
    f"weights_loss:{np.mean(temp_weights):.6f}  "
    f"log_mean:{log_sigma_means.item():.4f}  "
    f"log_std:{log_sigma_stds.item():.4f}  "
    f"log_weight:{log_sigma_weights.item():.4f}  "
    f"LR:{pa:.6f}"
)

    # Save model
    model_name = f'{n_neurons}_neurons_alpha_{alpha}_siren_model.pth'
    ...
    print(f"\n[{n_neurons} neurons | alpha = {alpha}] Avg PSNR: {avg_psnr:.6f} | RMSE: {rmse:.6f}")

    # Store results
    all_total_losses[alpha] = total_loss_list
    all_psnrs[alpha] = avg_psnr
    all_weight_losses[alpha] = weight_loss_list
    ...
    plt.title(f'Loss vs Epochs (n_neurons = {n_neurons}, alpha = {alpha})')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(outpath, f'loss_plot_{n_neurons}_neurons_alpha_{alpha}.png'))
    plt.close()


# ========== Run Training for All Alpha Settings ==========
for neurons in neuron_settings:
    train_and_evaluate(neurons, alpha_settings[0])

# ========== Comparison Plot: Total Loss vs Epoch (for all alphas) ==========
plt.figure(figsize=(12, 6))
for alpha in alpha_settings:
    plt.plot(all_total_losses[alpha], label=f'alpha={alpha}')
plt.title('Total Loss vs Epochs Comparison (Alpha)')
plt.xlabel('Epochs')
plt.ylabel('Total Loss')
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(outpath, 'total_loss_comparison_alpha.png'))
plt.close()

# ========== Comparison Plot: Weight Loss vs Epoch (for all alphas) ==========
plt.figure(figsize=(12, 6))
for alpha in alpha_settings:
    plt.plot(all_weight_losses[alpha], label=f'alpha={alpha}')
plt.title('Weight Loss vs Epochs Comparison (Alpha)')
plt.xlabel('Epochs')
plt.ylabel('Weight Loss')
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(outpath, 'weight_loss_comparison_alpha.png'))
plt.close()

# ========== Print Summary ==========
print("\n===== Final PSNR Summary =====")
print("\n=== Summary: Avg PSNR by Alpha ===")
for alpha, avg_psnr in all_psnrs.items():
    print(f"alpha = {alpha}: Avg PSNR = {avg_psnr:.4f}")
