...
args = Namespace(
    n_neurons=200,
    n_layers=6,
    epochs=500,  # Required argument: Set the number of epochs
    batchsize=10192,
    lr=0.00005,
    no_decay=False,
    decay_rate=0.8,
...
for alpha in range(1,31,5):
    print(f"\n{'='*40}\nTraining with {alpha} value\n{'='*40}", flush=True)
    ...
    for epoch in range(MAX_EPOCH):
	    ...
	    # === Forward Pass ===
            ...
            # === Apply softmax to last 3 outputs ===
            #predictions = torch.cat([predictions[:, :6],F.softmax(predictions[:, 6:], dim=1)], dim=1)
    ...
    final_model_name = f'siren_compressor_{alpha}n'
    torch.save(
    ...
    print(f"\n[PSNR] Evaluation for {alpha } value:", flush=True)
    ...
    print(f"\n[Weight Metrics] Evaluation for {alpha} value:", flush=True)
