...
'''defining the siren model with classes 1.sine layer,2.residual block,3.myresidual network'''
...
def compute_PSNR(arrgt,arr_recon):
    '''compute psnr of the predicted values'''
...
def findMultiVariatePSNR(var_name, total_vars, actual, pred):
    # print('Printing PSNR')
    '''fint the psnr of all the variables in the given list'''
    tot = 0
    psnr_list = []
...
def compute_rmse(actual, predicted):
    '''compute rmse of all the values combined'''
...
args = Namespace(
    ...
    decay_interval=15,
    datapath='data/new_GT_teardrop_GMM_week_11.vti',  # Required: Set the path to your data
    outpath='./models/',
    exp_path='../logs/',
    modified_data_path='./data/',
    dataset_name='3d_data',  # Required: Set the dataset name
    vti_name='predicted_GT_teardrop_GMM.vti',  # Required: Name of the dataset
    vti_path='./data/'
)
...
for name, indices in output_groups.items():
    ...
    for epoch in tqdm(range(MAX_EPOCH)):
        ...
        # Learning rate decay
        if decay:
            if  not decay_at_equal_interval and epoch >= decay_interval and epoch % decay_interval == 0:
