...
# Normalize inputs (x, y, z) to [0, 1]
X = df[["x", "y", "z"]].values
X_min = X.min(axis=0)
X_max = X.max(axis=0)
X_norm = (X - X_min) / (X_max - X_min)

# Normalize output (value) to [-1, 1]
...
writer.SetFileName("Isabel_3D_Predicted_Best_leaky.vti")
writer.SetInputData(output_image)
writer.Write()

print("Best model predictions saved as 'Isabel_3D_Predicted_Best_leaky.vti'")
