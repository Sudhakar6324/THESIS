# -*- coding: utf-8 -*-
"""parameter_learning_main_wweek_7 (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XXI1NUVB2tg5OppWubC5CCBGtldKHylW
"""
...
class MyResidualSirenNet(nn.Module):
    def __init__(self, obj):
        ...
        self.output_dim = obj['total_vars']  # should be 9
    ...
    def forward(self, x):
        ...
        # x: [B, 9] ? split: [means(3), stds(3), weights_logits(3)]
        return x # [B, 9]
...
def size_of_network(n_layers, n_neurons, d_in, d_out, is_residual = True):
    ...
    return n_params
def remove_sinusoidal_noise_volume(volume, frequency=10, amplitude=0.2):
    """
    Remove structured sinusoidal noise from a 3D volume.
    The noise is assumed to be added along the X-direction and constant across Y and Z.
    """
    z, y, x = volume.shape
    noise_pattern = amplitude * np.sin(np.linspace(0, frequency * 2 * np.pi, x))
    noise_2d = np.tile(noise_pattern, (y, 1))  # (Y, X)

    cleaned_volume = volume.copy()
    for zi in range(z):
        cleaned_volume[zi] -= noise_2d  # Subtract same noise in each Z-slice

    return cleaned_volume
def compute_PSNR(arrgt,arr_recon):
    diff = arrgt - arr_recon
    sqd_max_diff = (np.max(arrgt)-np.min(arrgt))**2
    print('rmse',np.mean(diff*2))
...
def compute_rmse(actual, predicted):
    ...
    return np.sqrt(mse)
def compute_rmse_all(total_vars,var_name,actual,predicted):
    rmse_all=[]
    tot=0
    for i in range(total_vars):
        rmse= compute_rmse(actual[:,i], predicted[:,i])
        rmse_all.append(rmse)
        tot += rmse
        print(var_name[i], ' PSNR:', rmse)
    return tot
def denormalizeValue(total_vars, to, ref):
    ...
    for i in range(total_vars):
        ...
        to_arr[:, i] = to[:, i] * (max_data - min_data) + min_data
...
args = Namespace(
    ...
    decay_interval=15,
    datapath='noise_gmm_week_8.vti',  # Required: Set the path to your data
    outpath='./models/',
    exp_path='../logs/',
    modified_data_path='./data/',
    dataset_name='3d_data',  # Required: Set the dataset name
    vti_name='predicted_isabel_7.vti',  # Required: Name of the dataset
    vti_path='./data/'
)
...
# ========== Config ==========
neuron_settings = [150,300,350,400,450,500]
all_total_losses = {}
all_psnrs = {}
univariate = True  # <- set True if output is 1D

# ========== Training Function ==========
def train_and_evaluate(n_neurons):
    ...
    # Track losses
    total_loss_list, entropy_loss_list = [], []
    mean_loss_list, std_loss_list, weight_loss_list = [], [], []
    ...
    for epoch in tqdm(range(MAX_EPOCH)):
        model.train()
        temp_total, temp_entropy = [], []
        temp_means, temp_stds, temp_weights = [], [], []
        ...
        for X_train, y_train in train_dataloader:
            ...
            pred_means, pred_stds, pred_weights = predictions[:, :3], predictions[:, 3:6], predictions[:, 6:9]
            gt_means, gt_stds, gt_weights = y_train[:, :3], y_train[:, 3:6], y_train[:, 6:9]

            loss_means = criterion(pred_means, gt_means)
            loss_stds = criterion(pred_stds, gt_stds)
            loss_weights = criterion(pred_weights, gt_weights)
            total_loss = (loss_means + loss_stds + loss_weights) / 3
            total_loss.backward()
            optimizer.step()

            temp_total.append(total_loss.item())
            temp_means.append(loss_means.item())
            temp_stds.append(loss_stds.item())
            temp_weights.append(loss_weights.item())

        epoch_loss = np.mean(temp_total)
        epoch_entropy = np.mean(temp_entropy)
        ...
        if decay:
            ...
            elif epoch > 0 and epoch_loss > total_loss_list[-1]:
                for param_group in optimizer.param_groups:
                    param_group['lr'] *= decay_rate
        pa = optimizer.param_groups[0]['lr']
        total_loss_list.append(epoch_loss)
        entropy_loss_list.append(epoch_entropy)
        mean_loss_list.append(np.mean(temp_means))
        std_loss_list.append(np.mean(temp_stds))
        weight_loss_list.append(np.mean(temp_weights))

        if epoch_loss < best_loss:
            best_loss = epoch_loss
            best_model = model.state_dict()
            best_epoch = epoch + 1
        print(f'epoch:{epoch} LOSS:{epoch_loss} LR:{pa}')
        print(f'\n mean_loss:{np.mean(temp_means)} stds_loss:{np.mean(temp_stds)} weights_loss:{np.mean(temp_weights)}')
    # Save model
    model_name = f'{n_neurons}_neurons_siren_model.pth'
    os.makedirs(outpath, exist_ok=True)
    torch.save({"epoch": best_epoch, "model_state_dict": best_model}, os.path.join(outpath, model_name))
    ...
    with torch.no_grad():
        for i in range(0, torch_coords.shape[0], group_size):
            ...
            vals = model(coords).cpu()
            for j in range(total_vars):
                prediction_list[j].append(vals[:, j])
    extracted_list = [np.concatenate([pred.detach().numpy() for pred in prediction_list[j]], dtype='float32') for j in range(total_vars)]
    n_predictions = np.array(extracted_list).T
    # Inference
    # Step 1: Denormalize predictions
    n_predictions = denormalizeValue(total_vars-3, n_predictions, real_data)

    # Step 2: Reshape predictions to 3D (Z, Y, X, D)
    dims = data.GetDimensions()      # (X, Y, Z)
    x, y, z = dims
    n_voxels = x * y * z
    assert n_predictions.shape[0] == n_voxels, "Prediction count does not match volume size"

    # Reshape to (Z, Y, X, D)
    n_predictions_3d = n_predictions.reshape((z, y, x, total_vars))

    # Step 3: Remove noise from GMM_Weight channels only (assume last 3 vars are weights)
    for i in range(total_vars - 3, total_vars):
        vol = n_predictions_3d[:, :, :, i]
        vol_clean = remove_sinusoidal_noise_volume(vol, frequency=8, amplitude=0.3)
        n_predictions_3d[:, :, :, i] = vol_clean

    # Step 4: Flatten back to (N, D) for metric computation
    n_predictions = n_predictions_3d.reshape((n_voxels, total_vars))

    # Step 5: Compute metrics
    psnr_list, avg_psnr = findMultiVariatePSNR(var_name, total_vars, real_data, n_predictions)
    rmse = compute_rmse_all(total_vars,var_name,real_data, n_predictions)
    print(f"\n[{n_neurons} neurons] Avg PSNR: {avg_psnr:.6f} | RMSE: {rmse}")

    # === Plot: loss curves ===
    plt.figure(figsize=(10, 6))
    plt.plot(total_loss_list, label='Total Loss')
    plt.plot(mean_loss_list, label='Mean Loss')
    plt.plot(std_loss_list, label='Std Loss')
    plt.plot(weight_loss_list, label='Weight Loss')
    plt.title(f'Loss vs Epochs (n_neurons = {n_neurons})')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(outpath, f'loss_plot_{n_neurons}_neurons.png'))
    plt.close()

    # === Plot: univariate prediction vs GT ===
    if univariate:
        plt.figure(figsize=(12, 5))
        plt.plot(n_val[:500], label="Ground Truth", linewidth=1)
        plt.plot(n_predictions[:500], label="Prediction", linewidth=1)
        plt.title(f"Prediction vs Ground Truth (n_neurons={n_neurons})")
        plt.xlabel("Sample")
        plt.ylabel("Value")
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(outpath, f'pred_vs_gt_{n_neurons}_neurons.png'))
        plt.close()

# ========== Run Training for All Neuron Settings ==========
for n in neuron_settings:
    train_and_evaluate(n)

# ========== Comparison Plot: Total Loss vs Epoch ==========
plt.figure(figsize=(12, 6))
for n in neuron_settings:
    plt.plot(all_total_losses[n], label=f'{n} neurons')
plt.title('Total Loss vs Epochs Comparison')
plt.xlabel('Epochs')
plt.ylabel('Total Loss')
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(outpath, 'total_loss_comparison.png'))
plt.close()

# ========== Print Summary ==========
print("\n===== Final PSNR Summary =====")
print("\n=== Summary: Avg PSNR by Neuron Count ===")
for n, avg_psnr in all_psnrs.items():
    print(f"{n} neurons: Avg PSNR = {avg_psnr:.4f}")
